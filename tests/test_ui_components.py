...
class TestImageWidgetReply:
    ...
    def test_image_widget_alt_click_emits_reply_requested(self):
        ...
        assert ev.caption is None  # display placeholder ≠ wire caption
        assert ev.timestamp == 1000
        assert ev.sender == "Mario"
        assert ev.is_mine is False
        assert ev.message_id == "msg-1"
        assert ev.attachment_info == "photo.jpg"
        assert ev.attachment_id == "att-1"

    def test_image_widget_alt_click_caption_preferred(self):
        """La caption reale ha priorità sul segnaposto nella reply."""
        ...
        assert events[0].caption == "Che bella!"  # wire text = real caption
    ...
    def test_image_widget_alt_r_action(self):
        ...
        assert events[0].caption is None
