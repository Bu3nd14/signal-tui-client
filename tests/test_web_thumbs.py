...
(async () => {
  ...
  const stage = helper + "\n" + app.slice(app.indexOf("async function stageAttachments("), app.indexOf("\nasync function submitMessage"));
  ...
  // La preview e' DOM puro: la si bypassa (qui si testa solo lo staging).
  globalThis.renderAttachmentsPreview = () => {};
  const last = () => state.stagedAttachments.at(-1);
  await stageAttachments([{ type: "image/png", size: 1024 * 1024, name: "large.png", width: 1500, height: 1000 }]);
  assert.equal(last().file.type, "image/jpeg");
  assert.ok(last().file.size < 1024 * 1024 * 0.4);
  const smallPng = { type: "image/png", size: 300 * 1024, name: "small.png", width: 1500, height: 1000 };
  await stageAttachments([smallPng]);
  assert.equal(last().file, smallPng);
  const smallJpeg = { type: "image/jpeg", size: 200 * 1024, name: "small.jpg", width: 1500, height: 1000 };
  await stageAttachments([smallJpeg]);
  assert.equal(last().file, smallJpeg);
  const largeJpeg = { type: "image/jpeg", size: 2 * 1024 * 1024, name: "large.jpg", width: 2500, height: 1000 };
  await stageAttachments([largeJpeg]);
  assert.notEqual(last().file, largeJpeg);
  assert.equal(last().file.type, "image/jpeg");
  const gif = { type: "image/gif", size: 1000, name: "animated.gif", width: 320, height: 240 };
  await stageAttachments([gif]);
  assert.equal(last().file, gif);
  const webp = { type: "image/webp", size: 1000, name: "image.webp", width: 640, height: 480 };
  await stageAttachments([webp]);
  assert.equal(last().file, webp);
  ...
  assert.equal(state.stagedAttachments.length, 6);

  globalThis.createImageBitmap = async () => { throw new Error("invalid image"); };
  const stagedBeforeInvalidImage = state.stagedAttachments.length;
  await stageAttachments([{ type: "image/gif", size: 1000, name: "invalid.gif" }]);
  assert.equal(state.stagedAttachments.length, stagedBeforeInvalidImage);
