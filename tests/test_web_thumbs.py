...
(async () => {
  ...
  let bitmapCalls = 0;
  globalThis.createImageBitmap = async () => { bitmapCalls += 1; return { width: 1000, height: 1000, close() {} }; };
  globalThis.document = { createElement: () => ({ getContext: () => ({ fillRect() {}, drawImage() {}, set fillStyle(_value) {} }), toBlob: (cb) => cb({ size: 100 }) }) };
  globalThis.URL.createObjectURL = () => "blob:preview";
  state.sending = false;
  vm.runInThisContext(stage);
  await stageAttachment({ type: "image/png", size: 3 * 1024 * 1024, name: "large.png" });
  assert.equal(state.stagedAttachment.file.type, "image/jpeg");
  assert.ok(state.stagedAttachment.file.size < 3 * 1024 * 1024 * 0.5);
