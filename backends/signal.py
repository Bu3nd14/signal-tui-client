...
class SignalBackend(ChatBackend):
    ...
    def _connect_sync(self) -> None:
        ...
        # start fresh with ``--receive-mode on-start``.  Otherwise the
        # daemon's SSE stream is connection-scoped: messages received by
        # the daemon during the TUI's absence have already been delivered
        # (to nobody) and will never be replayed.  The only reliable way
        # to fetch pending messages is a daemon restart.
